...
import torchvision
import torch
from data.utils import denormalize, normalize
...
class VGGPreTrained(PretrainNet):
    ...
    """
        ...
        self.mean: torch.Tensor = torch.tensor([0.485, 0.456, 0.406]).reshape(1, 3, 1, 1)
        self.std = torch.tensor([0.229, 0.224, 0.225]).reshape(1, 3, 1, 1)
        self.freeze()
        del vgg

    def _process(self, x):
        return normalize(denormalize(x), self.mean, self.std)
...
class VGGCaffePreTrained(nn.Module):
    ...
    def __init__(self,
                 weights_path: str = 'models/vgg19.npy',
        ...
        try:
            ...
            pass
        self.output_index = output_index
        self.mean: torch.Tensor = torch.tensor([103.939, 116.779, 123.68]).reshape(1, 3, 1, 1)
        self.vgg_normalize = lambda x: x - self.mean.to(x.device)

    def _process(self, x):
        # NOTE 图像范围为[-1~1]，先denormalize到0-1再归一化
        rgb = denormalize(x) * 255  # to 0~255
            ...
            if v == 'M':
                ...
                with torch.no_grad():
                    ...
                    weight = torch.FloatTensor(
                        self.get_conv_filter(data_dict,
                                             f'conv{block}_{number}'))
