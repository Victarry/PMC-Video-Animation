...
class Add():
    """remove datamean for numpy array
    """
    def __init__(self, y: List):
        super().__init__()
        self.y = np.array(y, dtype=np.float32) # (3)
    
    def __call__(self, x: np.array):
        output = np.clip(x+self.y, 0, 255).astype(np.uint8)
        return output
...
class ResizeToScale(Resize):
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(size={0},scale={1},interpolation={2})'.format(
