...
class MergeDataset(data.Dataset):
    ...
    def __getitem__(self, indexs: List[int]):
        return tuple(dataset[idx]
                     for idx, dataset in zip(indexs, self.datasets))
...
class DistributedSamplerWrapper(data.DistributedSampler):
    def __init__(self,
        ...
        super(DistributedSamplerWrapper,
              self).__init__(sampler.data_source, num_replicas, rank, shuffle)
...
class MultiBatchSampler(data.Sampler):
    ...
    def __init__(self, samplers: list, repeats: list, batch_size, drop_last=True):
        ...
        assert len(samplers) == len(
            repeats), 'Samplers number must equal repeats number'

        minweight = min(repeats)
        minlength = len(samplers[repeats.index(minweight)])
        self.sampler_loop = cycle(
            [i for i, w in enumerate(repeats) for _ in range(w)])
        # expand to target length
        self.repeats = repeats
        self.sizes = [minlength * math.ceil(w / minweight) for w in repeats]
