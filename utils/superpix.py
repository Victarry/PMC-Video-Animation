...
def slic(image, seg_num=200):
    seg_label = segmentation.slic(image,
                                  n_segments=seg_num,
                                  sigma=1,
                                  compactness=10,
                                  convert2lab=True,
...
def adaptive_slic(image, seg_num=200, power=1.2):
    seg_label = segmentation.slic(image,
                                  n_segments=seg_num,
                                  sigma=1,
                                  compactness=10,
...
def adaptive_label2rgb(label_field,
                       image,
                       kind='mix',
                       bg_label=-1,
                       bg_color=(0, 0, 0)):
...
class HierarchicalGrouping(object):
            ...
            for j in neighbors:
                if i < j:
                    self.s[(i, j)] = measure.calculate_sim(
                        self.regions[i], self.regions[j], self.img.size,
                        self.sim_strategy)
    ...
    def merge_region(self, i, j):
        ...
        new_box = (min(ri['box'][0],
                       rj['box'][0]), min(ri['box'][1], rj['box'][1]),
                   max(ri['box'][2],
                       rj['box'][2]), max(ri['box'][3], rj['box'][3]))
        value = {
            'box':
            new_box,
            'size':
...
def sscolor(image,
    ...
    img_seg = segmentation.felzenszwalb(image,
                                        scale=k,
                                        sigma=0.8,
                                        min_size=100)
